...
MODEL_ARCHITECT = "gemini-3.0-flash-preview" 
...
DB_FILE = "factory_run_gemma_v2.db"

# ==========================================
# 1. 文体定義 & Prompt Assets (完全版)
# ==========================================
STYLE_DEFINITIONS = {
...
作品設定、前半パートである**第1話〜第25話**の詳細プロット、マーケティングアセットを作成せよ。1話当たりプロットは500字とする。
...
後半の展開を劇的に、かつ整合性が取れるように作成せよ。1話当たりプロットは500字とする。
...
# ----------------------------------------------------------------
# 2. Episode Writing & QA Loop (GEMMA 3 ONLY)
# ----------------------------------------------------------------
# ----------------------------------------------------------------
    ...
    async def write_episodes(self, book_id, plots, mc_profile, style_dna):
        ...
        async def _worker(plot):
            async with semaphore:
                ...
                # コンテキスト取得 (並列時は直前の確定情報を取れない可能性があるため、Bible全体を信頼する)
                ...
                while retry_count <= max_retries:
                    ...
                    try:
                        ...
                        if (qa.cliffhanger_score < 70 or qa.kakuyomu_appeal_score < 70) and retry_count < max_retries:
                            print(f"    ⚠️ Low Quality Ep{ep_num}. Rewriting...")
...
# 2. Phase 1 Writing (Gemma 27B Loop)
...
# 4. Phase 2 Writing (Gemma 27B Loop)
