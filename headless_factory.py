...
【小説としての問題点と改善命令】
1. 定型句の機械的な反復（特に「引き」の演出）の回避
   - 【構成と語彙の多様化指示】: 各章の終わり方（クリフハンガー）のパターンを毎回変えてください。「謎の視線」や「不穏な気配」による引きは、物語全体で重要な転換点となる1回のみ使用し、それ以外は禁止します。
   - また、同じ形容詞や状況説明（例：「効率的」「単調」）を頻繁に使用せず、具体的な動作や比喩表現に置き換えて、表現の重複を避けてください。

2. 「天才」設定の根拠が薄い（Show, Don't Tellの欠如）の回避
   - 【具体的描写と論理性の強化指示】: 主人公の能力や賢さを、周囲の評価（セリフやコメント）だけで表現することを禁止します。
   - 「なぜその方法が画期的か」というロジックを具体的かつ専門的なプロセスとして描写してください。
   - 結果（倒したこと）だけを書くのではなく、思考プロセス（仮説・検証・実行）を詳細に描写し、読者が納得できる「解決策」を提示してください。

3. 場面転換が唐突で、ダイジェスト感が強いことの回避
   - 【シーンの深掘りと没入感の指示】: あらすじのような要約的な記述を避け、一つ一つのシーンを五感（視覚、聴覚、嗅覚など）を使って詳細に描写してください。
   - シーンの切り替えには必然性を持たせ、前のシーンの感情や余韻を次のシーンに繋げてください。
   - 単調なイベントの羅列ではなく、キャラクターの感情の揺れ動きや、場の空気感の変化に焦点を当てて執筆してください。
"""
