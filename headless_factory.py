...
【主人公の主体性回復（Agency Rule）】
各エピソードのクライマックスにおいて、以下のルールを厳守してください。
1. デウス・エクス・マキナの制限: ヒロインや「偶然」による解決は、トラブルのきっかけに留め、最終的な解決は必ず主人公の能動的アクションで行うこと。
...
1. 抽象語の禁止: 「すごい」「圧倒的」「美しい」「恐怖」という単語を直接使用することを禁止します。
2. 五感描写: 視覚だけでなく、聴覚（音）、嗅覚（血や錆の匂い）、触覚（温度、痛み）のいずれかを含む比喩表現を、1段落に最低1つ盛り込んでください。
3. Show, Don't Tell: 「彼は恐怖した」ではなく、「彼の指先は震え、呼吸は浅く引きつった」と描写してください。
